			...
			<view class="citys" v-if="!serachCity">
				<view v-for="(city, index) in sortItems" :key="index" v-show="city.isCity" class="citys-row">
					...
					<view class="citys-item" v-for="(item, inx) in city.citys" :key="inx" @click="cityTrigger(item)">
						{{ item.nameCN}}/{{ item.cityName }}</view>
				</view>
			</view>
			<!-- 国家列表(搜索后)  -->
			<view class="citys" v-if="serachCity">
				<view v-for="(item, index) in searchDatas" :key="index" class="citys-row">
					<view class="citys-item" :key="inx" @click="cityTrigger(item)">{{ item.name }}/{{ item.nameCN }}
					</view>
				</view>
			</view>
		</scroll-view>
...
<script>
	import areaSelect from './data.js';
	...
	export default {
		...
		computed: {
			 ...
			 */
			sortItems() {
				console.log('handleCity', this.handleCity)
			...
			searchDatas() {
				...
				for (let i = 0; i < this.cityData.length; i++) {
					if (this.cityData[i][this.formatName].indexOf(this.serachCity) !== -1 || this.cityData[i].nameCN.indexOf(this.serachCity) !== -1 ) {
						searchData.push({
							oldData: this.cityData[i],
							nameCN: this.cityData[i].nameCN,
		...
		methods: {
			...
			initializationCity() {
				...
				for (let index = 0; index < cityLetterArr.length; index++) {
					this.handleCity.push({
						...
						isCity: false, // 用于区分是否含有当前字母开头的国家
						citys: [], // 存放国家首字母含是此字母的数组
			...
			getLetter(str) {
				...
				// return areaSelect.getFirstLetter(str[0]);
			},
			/**
			 * @desc 构建国家索引
			 * @return undefind
			...
			buildCityindexs() {
				...
				for (let i = 0; i < this.cityData.length; i++) {
					// 获取首字母
					const cityLetter = this.getLetter(this.cityData[i][this.formatName])[0];
					// 获取当前国家首字母的unicode，用作后续排序
			...
			/**
			 * @desc 获取国家首字母的unicode
			 * @param letter String 国家索引
			 */
