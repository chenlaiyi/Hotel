<template>
	<view class="fui-tabbar" :class="{ 'fui-tabbar-fixed': isFixed, 'fui-unlined': unlined }"
		:style="{ background: backgroundColor }">
		<block v-for="(item, index) in tabBar" :key="index">
			<view class="fui-tabbar-item" :class="{ 'fui-item-hump': item.hump }"
				...
				<view class="fui-icon-box" :class="{ 'fui-tabbar-hump': item.hump }">
					<image :src="current == index ? item.selectedIconPath : item.iconPath"
						:class="[item.hump ? '' : 'fui-tabbar-icon']"></image>
					<view :class="[item.isDot ? 'fui-badge-dot' : 'fui-badge']"
						:style="{ color: badgeColor, backgroundColor: badgeBgColor }" v-if="item.num">
						{{ item.isDot ? '' : item.num }}
					</view>
				</view>
				<view class="fui-text-scale" :class="{ 'fui-text-hump': item.hump }"
