<template>
	<view class="fui-tabs-view" :class="[isFixed?'fui-tabs-fixed':'fui-tabs-relative',unlined?'fui-unlined':'']" :style="{height:height+'rpx',padding:`0 ${padding}rpx`,background:backgroundColor,top:isFixed?top+'px':'auto'}">
		<view v-for="(item,index) in tabs" :key="index" class="fui-tabs-item" :style="{width:itemWidth,borderRight:(index < (tabs.length-1))?borderRight:''}" @tap.stop="swichTabs(index)">
			<view class="fui-tabs-title" :class="{'fui-tabs-active':currentTab==index,'fui-tabs-disabled':item.disabled}" :style="{color:currentTab==index?selectedColor:color,fontSize:size+'rpx',lineHeight:lineHeight+'rpx',fontWeight:bold && currentTab==index?'bold':'normal'}">{{item.name}}</view>
		</view>
