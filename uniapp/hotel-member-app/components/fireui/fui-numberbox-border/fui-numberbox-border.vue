<template>
	<view class="fui-numberbox">
		<view class="padding-left-sm" :class="[disabled || value >= max ? 'fui-disabled' : '']" @tap="reduce"  :style="{ color: iconColor, fontSize: iconSize + 'rpx' }">-</view>
		<input
			type="number"
			v-model="inputValue"
			:disabled="disabled"
		...
		/>
        <view class="padding-right-sm" :class="[disabled || min >= value ? 'fui-disabled' : '']"  @tap="plus"  :style="{ color: iconColor, fontSize: iconSize + 'rpx' }">+</view>
	</view>
</template>

<script>
export default {
	name: 'fuiNumberboxBorder',
	props: {
		value: {
			type: Number,
			default: 1
