<template>
	<view class="fui-collapse margin-lr-sm radius-5" :style="{backgroundColor:bgColor}">
		<view class="fui-collapse-head" :style="{backgroundColor:hdBgColor}" @tap.stop="handleClick">
			<view class="fui-header" :class="{'fui-opacity':disabled}">
				...
				<view class="fui-collapse-icon fui-icon-arrow" :class="{'fui-icon-active':isOpen}" :style="{color:arrowColor}" v-if="arrow"></view>
			</view>
		</view>
		<view class="fui-collapse-body_box" :style="{backgroundColor:bdBgColor,height:isOpen?height:'0rpx'}">
			<view class="fui-collapse-body" :class="{'fui-collapse-transform':height=='auto','fui-collapse-body_show':isOpen && height=='auto'}">
