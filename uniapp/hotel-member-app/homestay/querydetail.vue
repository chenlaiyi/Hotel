<template>
	<view class="container">
		<!-- 民宿详情 -->
		<!-- #ifndef APP-PLUS -->
		<u-navbar title="民宿详情" color="#000000" :border="false" fixed placeholder :auto-back="true"></u-navbar>
		<!-- #endif -->
						...
						<view class="fui-flex flex-certer" @click="detailBtn(item)">
							...
							<view class="">
								...
								<view class="font_size_12 color6 text-bold">
									¥<text class="font_size_20">{{item.cprice}}</text>
								</view>
							</view>
						</view>
		...
		methods: {
			...
			getHotelDetail(hotel_id) {
				...
				that.$api.hotelMobileHotelDetail(hotel_id).then(res => {
					...
					that.tabBars = res.data.room_type.map((item, index) => {
						return {
							name: item.value,
							id: 'po' + item.key
						}
