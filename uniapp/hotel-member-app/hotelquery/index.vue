<template>
	<view class="container">
		<!-- #ifndef APP-PLUS -->
		<u-navbar title="首页"  fixed placeholder :auto-back="true" :border="true"></u-navbar>
		<!-- #endif -->
		...
		<view class="room_tj">
			<!-- 热门酒店 -->
			<hotel-list page="hotelIndex"></hotel-list>
			<!-- 周边景点 -->
	...
	export default {
		...
		methods: {
			initTimeStr() {
				...
				let today = new Date();
				console.log('initTimeStr-index', orderTime,today)
			...
			getPicture() {
				let that = this
				that.$api.hotelMobileIndexConfig().then(res => {
					that.topImg = res.data.index_thumb
				}).catch(res => {
					console.log('错误', res)
					uni.showToast({
						title: res.message,
