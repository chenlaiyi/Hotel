 ...
 * @Last Modified time: 2025-06-21 09:52:07
 */
...
class GlobalsService extends BaseService
{
    ...
    // 集团主营商户
    public int $global_store_id = 1;

    //模块标识
    private string $addons = 'sys';
    ...
    {
        $appId = Yii::$app->id;
        $cacheKey = 'common.conf_' . $appId . $bloc_id;
        $this->initUserType();
        loggingHelper::writeLog('GlobalsService', 'getConf', '全局配置获取开始', [
            'bloc_id' => $bloc_id,
            'appId' => $appId,
            'cacheKey' => $cacheKey
        ]);
    ...
    function initUserType(): void
    {
        if (!in_array(Yii::$app->id, ['app-console'])) {
            ...
            Yii::$app->params['userType'] = $userType;
        }
    }
