...
class AccessTokenService extends BaseService
{
     ...
     * @throws \Exception
     */
    public function getAccessToken(?DdMember $member, int $group_id=1, int $cycle_index = 1): array
    {
        ...
        if ($this->isPeriod($model->access_token) || empty($model->access_token)) {
            ...
            if ($this->isPeriodRefToken($model->refresh_token) || empty($model->refresh_token)) {
                ...
                $model->refresh_token = StringHelper::uuid('random').'_'.time();
            }
            $model->access_token = StringHelper::uuid('random').'_'.time();
            $model->status = 1;
            if (!$model->save()) {
                if ($cycle_index <= 3) {
                    ++$cycle_index;
        ...
        // 获取fans数据
        $result['wechatFans'] = DdWechatFans::find()->where(['member_id' => $member_id])->findBloc()->asArray()->one();
        $result['wxappFans'] = DdWxappFans::find()->where(['user_id' => $member_id])->findBloc()->asArray()->one();
    ...
    {
        try {
            loggingHelper::writeLog('AccessTokenService', 'forgetpassword', '忘记密码', [
                'member' => $member->id,
                'mobile' => $mobile,
                'password' => $password,
            ]);
            $member->setScenario('all');
            ...
            loggingHelper::writeLog('AccessTokenService', 'forgetpassword', '修改失败', [
                'error' => $e->getMessage(),
            ]);
