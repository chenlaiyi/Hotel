...
use common\services\admin\RegisterLevel;
...
class Fans extends BaseObject
{
    ...
    public function signup(?array $users): array|string
    {
        ...
        $openid = $this->validateOpenid($users['openid']);
        $nickname = $users['nickName'] ?? '';
        $mobile = $users['mobile'] ?? '';
        $keys = $openid . $mobile . '_userinfo';
        FileHelper::writeLog($logPath, '登录日志:用户信息openid' . json_encode([
                'openid' => $openid,
                'key' => $keys
            ]));
        ...
        try {
            if ($isHave) {
                FileHelper::writeLog($logPath, '登录日志:已有数据' . json_encode([
                        'mobile' => $mobile
                    ]));
                ...
                if ($mobile) {
                    /**
                     * 根据手机号查询，手机号多个，找到当前操作的会员id
                     */
                    $member_id = $DdMember::find()->where(['mobile' => $mobile])->select('member_id')->scalar();
                    FileHelper::writeLog($logPath, '登录日志:手机号获取会员数据' . json_encode([
                            'member_id' => $member_id,
                            'openid' => $openid,
                        ]));
                    if ($member_id) {
                        ...
                        FileHelper::writeLog($logPath, '不存在手机号，注册:' . json_encode([
                            'mobile' => $mobile
                        ]));
                        /**
                         * 不存在手机号，注册
                         */
                        $DdMember = new DdMember();
                        $DdMember->setScenario('mobile');
                        $password = 123456;
                        $max_member_id = $DdMember::find()->max('member_id');
                        $username = '用户' . $max_member_id + 1;

                        $Res = $DdMember->signup($username, $mobile, $password, $openid);
                        FileHelper::writeLog($logPath, '不存在手机号，注册结果:' . json_encode($Res));
                        $user_id = isset($res['data']['member']) ? $res['data']['member']['member_id'] : 0;
                        $fans->user_id = $user_id;
                        $fans->update();
                    }
                }

                $member = $DdMember::findIdentity($user_id);
                if (empty($member)) {
                    return ResultHelper::json(400, '用户不存在', $fans);
                }


                FileHelper::writeLog($logPath, '登录日志:会员数据' . json_encode([
                ...
                FileHelper::writeLog($logPath, '登录日志:组装用户信息' . json_encode($userinfo));
            ...
            FileHelper::writeLog($logPath, '登录日志：写入错误' . json_encode([
                    'msg' => $e->getMessage(),
                    'line' => $e->getLine()
                ]));
    ...
    public function checkByopenid($openid): array|YiiActiveRecord|null
    {
        return DdWxappFans::find()->where(['openid' => $openid, 'bloc_id' => \Yii::$app->request->input('bloc_id', 0)])->asArray()->one();
    }

    public function fansByopenid($openid): array|YiiActiveRecord|null
    {
        return DdWxappFans::find()->where(['openid' => $openid, 'bloc_id' => \Yii::$app->request->input('bloc_id', 0)])->one();
    }

    public function validateOpenid($openid)
    {
        // 使用正则表达式验证 openid 是否包含非法字符
        if (preg_match('/\s|(union|select|sleep|and|or|\(|\))/i', $openid)) {
            throw new InvalidArgumentException('Invalid openid detected.');
        }
        return $openid;
    }
