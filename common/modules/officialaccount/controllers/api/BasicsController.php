...
namespace common\modules\officialaccount\controllers\api;
...
{
    protected array $authOptional = ['signup', 'ceshi', 'auth', 'notify', 'userinfo'];
     ...
     */
    public function actionBindOpenid(): array
    {
        $code = Yii::$app->request->input('code');
        $member_id = Yii::$app->request->input('member_id');
        $wechat = Yii::$app->wechat->getApp();
        $user = $wechat->oauth->userFromCode($code)->getRaw();
         ...
         * 获取用户信息
         */
        $res = Yii::$app->fans->bindOpenid($member_id,$users);
        return ResultHelper::json(200, '登录成功', $res);
    }
    ...
    public function actionUserinfo(): array
    {
        ...
        $wechat = Yii::$app->wechat->app;
        $user = $wechat->oauth->userFromCode(Yii::$app->request->input('code'));
        if (empty($user)) {
            return ResultHelper::json(400, '用户信息获取失败', $user);
        } else {
            FileHelper::writeLog($logPath, '用户信息获取成功'.json_encode($user));
            ...
            $data['openid'] = $user['raw']['openid'];
            $data['country'] = $user['raw']['country'];
            $data['city'] = $user['raw']['city'];
            $data['province'] = $user['raw']['province'];

            $Res = Yii::$app->fans->signup($data);
    ...
    public function actionPayparameters(): array
    {
        ...
        if (empty($openid)){
            return ResultHelper::json(409, '非微信授权用户不能支付');
        }
        $data = Yii::$app->request->post();
        $profit_sharing = strtoupper($data['profit_sharing']??'N');
        ...
        $orderData = [
            ...
            'fee_type' => 'CNY',
            'profit_sharing'=>$profit_sharing,//增加分账参数测试
        ...
        if ($result['return_code'] == 'SUCCESS' && $result['result_code'] == 'SUCCESS') {
            ...
            return ResultHelper::json(401, '支付发起错误', $result);
        }
    }
