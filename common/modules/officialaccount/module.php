...
namespace common\modules\officialaccount;
...
class module extends \yii\base\Module
{
     ...
     * {@inheritdoc}
     */
    public $controllerNamespace = 'common\modules\officialaccount\controllers\api';
    ...
    public function init(): void
    {
        ...
        loggingHelper::writeLog('officialaccount', 'payparameters', '入口配置回来的值' . $input);
        loggingHelper::writeLog('officialaccount', 'payparameters', '入口配置回来的值0-5' . substr($input, 0, 5));
        if (str_starts_with($input, '<xml>')) {
            loggingHelper::writeLog('officialaccount', 'payparameters', '准备处理');
            $xmldata = StringHelper::getXml($input);
            loggingHelper::writeLog('officialaccount', 'payparameters', 'xml解析后' . $xmldata['trade_type'] . '/' . json_encode($xmldata));
            if ($xmldata['trade_type'] == 'JSAPI' || $xmldata['trade_type'] == 'APP') {
                $out_trade_no = $xmldata['out_trade_no'];
                loggingHelper::writeLog('officialaccount', 'payparameters', '入口配置回来的订单编号：' . $out_trade_no);
                ...
                loggingHelper::writeLog('officialaccount', 'payparameters', '入口配置回来的xml值订单日志sql' . $DdCorePaylog->find()->where([
                        'uniontid' => trim($out_trade_no),
                    ])->select(['bloc_id', 'store_id', 'module'])->createCommand()->getRawSql());
                loggingHelper::writeLog('officialaccount', 'payparameters', '入口配置回来的xml值订单日志' . json_encode($orderInfo));
        ...
        switch ($appId) {
            case 'app-admin':
                $this->controllerNamespace = 'common\modules\officialaccount\controllers\admin';
                ...
                break;
            case 'app-api':
                $this->controllerNamespace = 'common\modules\officialaccount\controllers\api';
     ...
     * @return array
     */
    public function initConf(): array
    {
        ...
        if (StringHelper::strExists($requestedRoute, 'payappparameters')) {
            ...
            loggingHelper::writeLog('officialaccount', 'payparameters', 'app支付' . json_encode($config['params']['wechatPaymentConfig']));
        } else {
        ...
        loggingHelper::writeLog('officialaccount', 'payparameters', '入口配置' . json_encode($config['params']['wechatPaymentConfig']));
        loggingHelper::writeLog('officialaccount', 'payparameters', '总配置' . json_encode($conf));
        ...
        loggingHelper::writeLog('officialaccount', 'moduleInit', '公众号配置' ,[
            'params'=>$params,
            'conf'=> $conf,
            'wechat'=> $wechat,
