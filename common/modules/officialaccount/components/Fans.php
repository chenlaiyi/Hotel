...
class Fans extends BaseObject
{
     ...
     */
    public function signup(?array $users): array
    {
        ...
        if (!empty($fans)) {
            ...
            $member = $DdMember::findIdentity($fans['member_id']);
            ...
            if (empty($nickname)) {
                $maxUid = DdMember::find()->select('member_id')->max('member_id');
                $nickname = '用户'.($maxUid+1);
            }
            $res = $DdMember->signup($nickname, '', $password, $openid);


            // 更新openid
            $member_id = isset($res['data']['member']) ? $res['data']['member']['member_id']:0;
            FileHelper::writeLog($logPath, '登录日志:获取用户id' . json_encode($res));
            ...
            if ($DdWechatFans->load($dataFans, '') && $DdWechatFans->save()) {
                ...
                return ResultHelper::json(422, $errors);
            }
        }
    }
    ...
    //bindOpenid
    public function bindOpenid($member_id,$users): array|YiiActiveRecord|null
    {
        loggingHelper::writeLog('officialaccount', 'bindOpenid', '绑定openid',[
            'member_id' => $member_id,
            'users' => $users
        ]);
        ...
        $model = $DdWxappFans::findOne(['openid' => $openid]);
        if ($model) {
            $model->user_id = $member_id;
            $model->save();
            $arr['fans'] = $model->toArray();

            return ResultHelper::json(200, '绑定成功', $arr);
        } else {
            $password = StringHelper::randomNum();
            $nickname = $users['nickname']?? '用户' . ($member_id + 1);
            DdApiAccessToken::updateAll(['openid' => $openid], ['member_id' => $member_id]);
            // 注册fans
            // 生成随机的加密键
            $secretKey = Yii::$app->getSecurity()->generateRandomString();
    ...
    public function checkByopenid($openid): array|\common\components\ActiveRecord\YiiActiveRecord|null
   {

        return DdWechatFans::find()->where(['openid' => $openid, 'bloc_id' =>\Yii::$app->request->input('bloc_id',0)])->asArray()->one();
    }

    public function fansByopenid($openid): array|\common\components\ActiveRecord\YiiActiveRecord|null
   {
    ...
    private function validateOpenid(mixed $openid)
    {
