...
class Sign extends ActionFilter
{
    ...
    public static function generateSecret(): string
    {
        ...
        if (Yii::$app->request->post('bloc_id')) {
            $bloc_id = Yii::$app->request->post('bloc_id');
        }
        $apiConf->getConf($bloc_id);
        loggingHelper::writeLog('sign', 'generateSecret', 'app_secret', [
            'app_secret' => $apiConf['app_secret'],
            'bloc_id' => $bloc_id,
            'bloc_id_post' => Yii::$app->request->post('bloc_id'),
    ...
    {
        // 生成sign  字符串和密钥拼接
        $str = urldecode($preStr)  . '&key=' . self::generateSecret();
    ...
    function getSign(array $params)
    {
        ...
        return $this->md5Sign($forAllString, $params['app_id']);
    }
