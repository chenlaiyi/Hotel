 ...
 */
class WebsiteArticle extends \common\components\ActiveRecord\YiiActiveRecord
{
    use StoreTrait;
    ...
    public function rules(): array
    {
        return [
            ...
            [['ishot', 'type', 'pcate', 'ccate',  'displayorder', 'create_time', 'update_time', 'bloc_id', 'store_id', 'is_top'], 'integer'],
            [['content'], 'string'],
    ...
    public function beforeSave($insert)
    {
        if (parent::beforeSave($insert)) {
            if ($this->pcate) {
                //字段
                $this->type = WebsiteArticleCategory::find()->where(['id' => $this->pcate])->select('page_id')->scalar();
            }
            return true;
        } else {
            return false;
