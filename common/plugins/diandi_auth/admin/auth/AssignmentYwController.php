...
use common\models\AuthAssignmentGroup;
 ...
 * @since 1.0
 */
class AssignmentYwController extends AController
{
    public $modelClass = '';
...
//        }
    ...
    {
        $admin          = User::find()->where(['id' => $id])->select(['is_super_admin', 'bloc_id'])->asArray()->one();
        ...
        $roleItemId = [];
        if ($is_super_admin === 0) {
            $bloc_id    = $admin[ 'bloc_id' ];
            $roleItemId = AuthUserGroup::find()->where(['bloc_id' => $bloc_id])->select('item_id')->asArray()->column();
        }
        $model     = $this->getAssigment($id);
        $all       = [];
        $assigneds = [];
        if ($model instanceof Assignment) {
            $items     = $model->getItems(0, $bloc_id);
            $all       = $items[ 'all' ];
        ...
        if($user_info->is_super_admin == 1){
            ...
            $store_ids = UserStore::find()->where(['user_id' => $user_id])->select('store_id')->asArray()->column();
        }

        $list = Bloc::find()->where(['bloc_id' => $bloc_id])->with([
                'store' => function ($query) use ($store_ids) {
                    if($store_ids) {
                        $query->andWhere(['store_id' => $store_ids]);
                    }
                }
            ]
        )->asArray()->all();
...
//            $lists[] = $value;
        ...
        if (!$is_super_admin) {
            $user_group = AuthAssignmentGroup::find()->where(['user_id' => $user_id])->select('group_id')->column();
            if ($user_group) {
                $top_level = AuthUserGroup::find()->where(['in', 'id', $user_group])
                    ->andWhere(['bloc_id' => $bloc_id])
                    ->orderBy('level asc')
                    ->select('level')
                    ->scalar();
                $not_role  = AuthUserGroup::find()
                    ->where(['bloc_id' => $bloc_id])
                    ->andWhere(['<', 'level', $top_level])
                    ->select('item_id')
                    ->column();
            }
        }
        ...
        foreach ($all as $key => $value) {
            foreach ($value as $item) {
                $item = (array) $item;
                if ($key === 'role' && $is_super_admin === 0) {
                    if ($item[ 'is_sys' ] === 1 || !in_array($item[ 'item_id' ], $roleItemId)) {
                        continue;
                    }
                }
                if ($key === 'role') {
                    if (in_array($item[ 'item_id' ], $not_role)) {
                        continue;
                    }
                }
                $alls[ $key ][] = $item;
        ...
        if (!empty($alls[ 'permission' ])) {
            $alls[ 'permission' ] = ArrayHelper::itemsMerge($alls[ 'permission' ], 0, 'id', 'parent_id', 'children');
        } else {
            $alls[ 'permission' ] = [];
        }
    ...
    public function actionChange(): array
    {
        ...
        $model = $this->getAssigment($id);
        if (!$model instanceof Assignment) {
            return ResultHelper::json(400, '找不到用户');
        }
...
//                $lists[] = $value;
        ...
        switch ($type) {
            ...
            case 'role':
                ...
                $diffArr = ArrayHelper::array2diff($assigned_ids, $authItems);
            ...
            case 'store':
                 ...
                 * 查询授权的公司ID集合
                 */
                $bloc_ids = UserStore::find()->where(['user_id' => $id])->select('bloc_id')->groupBy('bloc_id')->column();
                if ($bloc_ids){
                    foreach ($bloc_ids as $bloc_id) {
                        /**
                         * 查询公司是否授权，没有授权就授权处理
                         */
                        $have = UserBloc::find()->where(['user_id' => $id,'bloc_id' => $bloc_id])->exists();
                        if(!$have){
                            ...
                            $UserBloc->setAttributes($data);
                            if (!$UserBloc->save()) {
                                $msg = ErrorsHelper::getModelError($UserBloc);
                            }
                        }
                    }
                }else{
                    $bloc_id = DdUser::find()->where(['id' => $id])->select('bloc_id')->scalar();
                    /**
                     * 没有商户对应的公司授权，就保留用户本身的bloc_id
                     */
                    UserBloc::deleteAll([
                        'and',
                        [
...
//        }
        ...
        return ResultHelper::json(200, '更新完成' );
    }
