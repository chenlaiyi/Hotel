...
class StringHelper extends BaseStringHelper
{
    ...
    {
        if (!in_array($type, ['time', 'md5', 'random', 'sha1', 'uniqid'], true)) {
            throw new InvalidArgumentException("Unsupported UUID type");
        }
            ...
            switch ($type) {
                ...
                case 'md5':
                    if (!filter_var($name, FILTER_VALIDATE_URL)) {
                        throw new InvalidArgumentException("Invalid name for UUID type 'md5'");
                    }
                ...
                case 'sha1':
                    if (!filter_var($name, FILTER_VALIDATE_URL)) {
                        throw new InvalidArgumentException("Invalid name for UUID type 'sha1'");
                    }
                    $uuid = Uuid::uuid5(Uuid::NAMESPACE_DNS, $name);
                    break;
                case 'uniqid':
                    // 使用更安全的哈希算法SHA-256
                    $uuid = hash('sha256', uniqid(self::randomBytes(16), true));
                    break;
            }

            // 如果$uuid是一个对象，则调用toString方法；如果是字符串，则直接返回
            return is_object($uuid) ? $uuid->toString() : $uuid;
        } catch (Exception $e) {
            // 可以考虑记录错误或返回一个空的UUID，根据实际需要
            error_log("Error generating UUID: " . $e->getMessage());
            return '';
        }
    }

    // 强化随机数生成器，使用PHP的内置安全随机数生成器
    ...
    {
        $pattern = "/^1[123456789]\d{9}$/"; // 手机号的正则表达式
