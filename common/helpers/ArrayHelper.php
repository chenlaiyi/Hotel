...
class ArrayHelper extends BaseArrayHelper
{
     ...
     * @return bool|string
     */
    public static function itemsLevel(int $level, array $models, $k, int $treeStat = 1)
    {
        $str = '';
        for ($i = 1; $i < $level; ++$i) {
            $str .= '　　';
     ...
     * @return array
     */
    public static function itemsMergeDropDown($models, string $idField = 'id', string $titleField = 'title', int $treeStat = 1)
    {
        $arr = [];
     ...
     * @return mixed
     */
    public static function getFirstRowByItemsMerge(array $array): mixed
    {
     ...
     * @return mixed
     */
    public static function getNotChildRowsByItemsMerge(array $array): mixed
    {
        $arr = [];
     ...
     * @return mixed
     */
    public static function getRowsByItemsMerge(array $array, string $childField = '-')
    {
        $arr = [];
    ...
    public static function array2xml($arr, $level = 1): array|string|null
    {
        $s = 1 == $level ? '<xml>' : '';
    ...
    public static function xml2array($xml): array|string
    {
        if (empty($xml)) {
            return [];
        }
     ...
     * 比较2个二维数组的差异
     */
    public static function array2diff($oldArray, $newArray): array
    {
        // 确保输入是数组
        ...
        // 找出需要删除的数据（在旧数组中但不在新数组中的数据）
        $toDelete = array_diff($oldArray, $newArray);

        // 找出需要添加的数据（在新数组中但不在旧数组中的数据）
        $toAdd = array_diff($newArray, $oldArray);
