...
EOF;
    ...
    public static function adminSignUp($username, $mobile, $email, $password)
    {
        ...
        if (!empty($userinfo)) {
            self::resContent('数据库初始成功，下一步注册管理员');
        }
        /* 查看手机号是否重复 */
        if ($mobile) {
        ...
        if ($AuthAssignmentGroup->load($data, '') && $AuthAssignmentGroup->save()){
            return true;
        }else{
            $msg = ErrorsHelper::getModelError($AuthAssignmentGroup);
            loggingHelper::writeLog('install','adminSignUp','注册失败', [
                'msg'=> $msg
            ]);
            return false;
        }
    ...
    public static function getConf($key,$message)
    {
        ...
        return self::resContent($message.':');
    }

    public static function resContent($message)
    {
        if (Yii::$app->id == 'console'){
            return self::resContent($message);
        }else{
            return ResultHelper::json(404, $message);
        }
    }
}
