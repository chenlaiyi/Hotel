...
class Generator extends \yii\gii\Generator
{
    ...
    {
        return '这个生成器生成实现CRUD(创建、读取、更新、删除)的控制器。指定数据模型的操作。';
    }
    ...
    public function rules(): array
    {
        return array_merge(parent::rules(), [
            [['moduleID','controllerClass', 'modelClass', 'searchModelClass', 'baseControllerClass','indexName','createName','updateName'], 'filter', 'filter' => 'trim', 'skipOnEmpty' => true],
    ...
    public function generate(): array
    {
        ...
        $getPrimaryKey = (new $this->modelClass)->primaryKey();
        $pk = $getPrimaryKey[0];
        $pathApi = $this->getApiPath();
        $formType = [
            'int' => 'number',
            'datetime' => 'datetime',
            'varchar' => 'input',
        ...
        foreach ($attributeLabels as $prop=> $label) {
            ...
            if (!empty($TableSchemaArray[$prop])){
                ...
                if (!in_array($prop,[$pk,'create_time','update_time','bloc_id','store_id'])){
                    $form[$prop] = [
        ...
        $initContent = str_replace([
            ...
            json_encode($form,JSON_UNESCAPED_UNICODE),
            json_encode($order,JSON_UNESCAPED_UNICODE),
            json_encode($tableColumns,JSON_UNESCAPED_UNICODE),
            json_encode($fieldList,JSON_UNESCAPED_UNICODE),
            "'".$pk."'",
            "'".$pathApi."'",
            "'".$this->indexName."'",
            "'".$this->createName."'",
        ...
        $files[] = new CodeFile(
            $viewPath . '/create.vue',
            $this->render('vue/create.vue')
        );

        $files[] = new CodeFile(
            $viewPath . '/index.vue',
            $this->render('vue/index.vue')
        );

        $files[] = new CodeFile(
            $viewPath . '/update.vue',
            $this->render('vue/update.vue')
        );
    ...
    {
        //        addons\zyj_wash\admin\card\PriceController
        $class = str_replace('Controller','',$this->controllerClass);
    ...
    public function createInitParams(): array
    {
        return [
            'mm'=>123
        ];
    ...
    /**
     * @return string the controllers view a path
     */
    public function getViewPath(): string
    {
        ...
        if (empty($this->viewPath) && $path) {
            ...
            return $viewPath;
        }else if(empty($this->viewPath)){
            $viewPath = Yii::getAlias('@addons/' . $this->moduleID . '/views/' . $this->getControllerID());
            FileHelper::mkdirs($viewPath);
            return $viewPath;
        }
     ...
     */
    public function validateNewClass($attribute, $params): void
    {
     ...
     * @throws InvalidConfigException
     */
    public function getTableSchema(): bool|TableSchema
    {
        /* @var $class ActiveRecord */
        $class = $this->modelClass;
     ...
     * @since 2.0.6
     */
    protected function getClassDbDriverName(): ?string
    {
        /* @var $class ActiveRecord */
        $class = $this->modelClass;
        $db = $class::getDb();
