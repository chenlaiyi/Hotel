...
use common\services\api\AccessTokenService;
use Yii;
...
class DdMember extends ActiveRecord
{
    ...
    public function rules(): array
    {
        return [
            ...
            [['username', 'openid', 'invitation_code', 'nickName', 'avatarUrl', 'verification_token', 'address', 'email'], 'string', 'max' => 255],
    ...
    {
        if ($insert && empty($this->invitation_code)) {
            $this->invitation_code = HashidsHelper::encode($this->member_id);
            $this->update();
        }
    ...
    {
        return $this->hasOne(AccountLog::class, ['member_id' => 'member_id']);
    }
    ...
    public function signup($username, $mobile, $password,$openid=''): array|bool
    {
        ...
        if ($mobile) {
            ...
            $userinfo = $this->find()->where(['mobile' => $mobile,'bloc_id'=>$bloc_id])
                ->select(['member_id','openid'])->one();
            FileHelper::writeLog( $logPath, '登录日志:查询会员信息' . json_encode($userinfo));
            if (!empty($userinfo)) {
         ...
         */
        if ($openid){
            $userinfo = $this->find()->where(['openid' => $openid])->select('member_id')->one();
            if (!empty($userinfo)) {
                $member_id = $userinfo->member_id;
            }
        }
        ...
        try {
            ...
            if ($member_id){
                /**
                 * 根据member_id更新当前openid
                 */
                Yii::$app->db->createCommand()->update('{{%member}}', ['openid' => $openid], ['member_id' => $member_id])->execute();
                $model = self::find()->where(['member_id' => $member_id])->one();
                /* 写入用户apitoken */
                $AccessTokenService = new AccessTokenService();
                $member = $AccessTokenService->getAccessToken($model, 1);
                ...
                if ($memberRes) {
                    ...
                    /* 写入用户apitoken */
                    $AccessTokenService = new AccessTokenService();
                    $member = $AccessTokenService->getAccessToken($this, 1);
                    return ResultHelper::json(200,'注册成功',$member);
