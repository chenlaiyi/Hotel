 ...
 */
namespace addons\diandi_place\api\manage\landlord;
...
{
    use LandlordTrait;
    public array $adminAuth = ['identity','landlord'];
    public function actionInit(): array
    {
        $member_id = Yii::$app->user->identity->member_id ?? 0;
        $list = LandlordService::initAuth($member_id);
    ...
    public function actionLandlord(): array
    {
        ...
        $REs = LandlordService::landlordIdentity($member_id, $realname, $icard_code, $icard_front, $icard_back);
    ...
    public function actionSetType(): array
    {
        ...
        $REs = LandlordService::setType($member_id, $config);
        return ResultHelper::json(200, '设置成功', $REs);
    }
}
