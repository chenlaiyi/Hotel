<template>
  <div>
    ...
    >
      ...
      <el-form-item v-if="!this.$route.query.invitation_code" prop="invitation_code">
        <el-input
          ref="invitation_code"
...
export default {
  ...
  created() {
    ...
    if (this.$route.query.invitation_code) {
      this.$set(this.loginForm, 'invitation_code', this.$route.query.invitation_code)
    }
    if (this.$route.query.openid) {
      this.$set(this.loginForm, 'openid', this.$route.query.openid)
    }
  },
  methods: {
    login() {
    ...
    handleRegister() {
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          this.loading = true
          this.userSignup(this.loginForm)
            .then((res) => {
              if (res.code === 200) {
                this.loading = false
