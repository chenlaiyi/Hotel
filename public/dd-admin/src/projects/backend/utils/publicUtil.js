 ...
 * @Last Modified time: 2025-02-09 06:33:37
 */
...
service.interceptors.request.use(
  config => {
    ...
    if (store.getters.accessToken) {
      ...
      config.headers['bloc-id'] = Cookies.get('bloc-id') || store.getters.blocId
      config.headers['store-id'] = Cookies.get('store-id') || store.getters.storeId
  ...
  response => {
    ...
    if (resCode !== 200) {
      ...
      if (resCode === 402) {
        ...
        // token过期重新登录
        store.dispatch('user/refreshToken', refresh_token).then((res) => {
