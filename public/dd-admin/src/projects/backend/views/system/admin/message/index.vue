...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [
        { label: '标题', prop: 'title' },
        { label: '接收者IDS', prop: 'admin_ids' },
        { label: '发布时间', prop: 'publish_at' },
        { label: '查看次数', prop: 'view' },
      ...
      filterInfo: {
        ...
        fieldList: [
          { label: '标题', type: 'input', value: 'HubMessagesSearch[title]' }
        ]
      },
      listTypeInfo: {},
      total: 0,
  ...
  methods: {
    ...
    getList() {
      ...
      messagelistIndex(that.filterInfo.data).then((response) => {
    ...
    delectRow(row) {
      const that = this
      messagelistDelete(row.id).then((response) => {
