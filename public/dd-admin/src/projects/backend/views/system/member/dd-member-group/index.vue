...
import {
  ...
  deleteItem,
  fetchView
} from '@projectName/views/system/api/system/MemberGroup.js'
...
export default {
        ...
        fieldList: [
          {
            label: '会员组名称',
            type: 'input',
            value: 'DdMemberGroupSearch[item_name]'
          }
        ]
      },
      listTypeInfo: {},
  ...
  methods: {
    ...
    deleteItem(row) {
      const that = this
      deleteItem(row.group_id).then((res) => {
        console.log('更新', res)
        that.getList()
        that.dialogFormVisible = false
      })
    ...
    editItem(row) {
      ...
      fetchView(row.group_id).then((res) => {
