<template>
  <div class="app-container">
    ...
    <div v-if="current === 0" class="sub-store-order margin-top-sm">
      <div class="padding-bottom-sm text-bold text-xss">详情介绍</div>
      <p v-html="goodsdetail" />
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    getDetail() {
      ...
      getgoodsdetail(data).then((res) => {
        console.log('selling_point', res.data)
        that.goodsdetail = res.data.dis.addons.description
      })
    },
