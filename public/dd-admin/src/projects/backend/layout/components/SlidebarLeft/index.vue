<template>
  <div>
    ...
    <div v-if="Layout == 'subfield'" class="sidebar-main">
      ...
      <el-scrollbar
        class="subfield-sidebar-container"
        wrap-class="scrollbar-wrapper"
      >
        ...
        >
          ...
          <!-- 子插件菜单start -->
          <sidebar-item
            v-for="(route, index) in pluginsMenu"
            :key="route.path + isActive + index"
            :item="route"
            :base-path="route.path"
            :is-plugins="true"
...
export default {
  ...
  computed: {
    ...
    subLeftIsActive() {
      ...
      if (this.$store.state.permission.subLeftIsActive) {
        ...
        console.log('initLeftSubMenu', this.pluginsMenu, this.LeftMenu)
    ...
    activeMenu() {
      ...
      console.log('menuType', this.menuType, route.path)
      let id = 0
      // 支持插件单独使用也支持组合使用
  ...
  created() {
    ...
    console.log('menus-cre', menus, this.menuType, that.isActive)
    const id = menus.id
    that.isActive = id
    that.initLeftSubMenu(id)
