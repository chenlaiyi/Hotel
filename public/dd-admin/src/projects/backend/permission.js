 ...
 * @Last Modified time: 2025-02-09 06:37:31
 */
...
import {
  getToken
} from '@projectName/utils/auth' // get token from cookie
import { config } from '@projectName/utils/publicUtil'
import getPageTitle from '@projectName/utils/get-page-title'
import { getsignup } from '@projectName/api/user.js'
NProgress.configure({
  showSpinner: false
...
router.beforeEach(async(to, from, next) => {
  ...
  if (hasToken) {
    store.dispatch('elForm/changeSite', config.siteUrl, config.bmapAk)
    await store.dispatch('elForm/changeHEaders', {
      key: 'access-token',
      value: hasToken
    })
    if (to.path === '/login') {
      ...
      if (is_roles && permission_routes) {
        ...
        try {
          ...
          const {
            roles
          } = await store.dispatch('user/getInfo', window.location.host)
          // 动态生成路由
