...
export default {
      ...
      formConfig: {
        formDesc: {
          ...
          rule_name: {
            ...
            label: '规则名称',
            isOptions: true,
            options: this.getRules().then((res) => {
              return res.data
            })
          },
          parent_id: {
            ...
            label: '父级权限',
            isOptions: true,
            options: this.fetchLevels().then((res) => {
  ...
  methods: {
    ...
    deleteItem(row) {
      const that = this
      this.deletePermission(row.id).then((res) => {
        console.log('更新', res)
        that.getList()
    ...
    authItem(row) {
      ...
      this.fetchViewPermission(row.id).then((res) => {
    ...
    getList() {
      ...
      this.fetchListPermission(that.filterInfo.data).then((response) => {
