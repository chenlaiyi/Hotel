 ...
 * @property mixed|string|null $end_time
 */
class Bloc extends BlocModel
{
    ...
    {
        return $this->hasMany(Bloc::class, ['pid' => 'bloc_id']);
    }
    ...
    public function search(array $params): array|bool
    {
        ...
        $is_admin = UserService::isSuperAdmin();
        $bloc_ids = UserBloc::find()->select('bloc_id')->where(['user_id' => \Yii::$app->user->id])->select('bloc_id')->column();
        if ($bloc_ids && !$is_admin){
            $query->andWhere(['in', 'bloc_id', $bloc_ids]);
        }
        $count = $query->count();
