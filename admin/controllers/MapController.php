...
class MapController extends AController
{
    ...
    public function actionCitylist(): array
    {
        ...
        if ($regionVal && ! empty($regionVal)) {
            $citylist = $regionVal;
        } else {
            $query = Yii::$app->db->createCommand("SELECT `name`, `id`, `pid`, name as label, id as value FROM {{%region}}");
            $sql   = $query->getRawSql();
            $list  = $query->queryAll();

            // echo Yii::$app->db->createCommand("SELECT 'name', 'id', 'pid', 'name as label', 'id as value' FROM {{%region}}")->getRawSql();
