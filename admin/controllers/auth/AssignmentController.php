...
//        }
    ...
    public function actionView($id): array
    {
        ...
        $roleItemId =  [];
        /**
         * 不是超级管理员，只能查看自己所属商户的权限
         */
        if ($is_super_admin === 0){
            $bloc_id = $admin['bloc_id'];
            $roleItemId = AuthUserGroup::find()->where(['bloc_id' => $bloc_id])->orWhere(['name' => $defaultRoles])->select('item_id')->asArray()->column();
        }
...
//            $lists[] = $value;
        ...
        $assigneds['store'] = UserStore::find()->alias('u')
            ->joinWith('store as s')->where(['u.user_id' => $id, 's.store_id' => $store_ids])
            ->select('s.store_id')->indexBy('s.store_id')->column();
        ...
        foreach ($all as $key => $value) {
            foreach ($value as $item) {
                $item = (array) $item;
                /**
                 * 不是超级管理员，只能查看自己所属公司的权限
                 */
                if($key === 'role' && $item['is_sys'] !== 1){
                    continue;
//                    if (!in_array($item['item_id'], $roleItemId)){
//                        continue;
//                    }
                }
                $alls[$key][] = $item;
        ...
        if(!empty($alls['permission'])){
            $alls['permission'] = AuthService::authGroupPermission($alls['permission']);
        }else{
            $alls['permission'] = [];
        }
...
//                $lists[] = $value;
        ...
        switch ($type) {
            ...
            case 'store':
                ...
                $addList = BlocStore::find()->where(['store_id' => $authItems])->asArray()->all();
...
//                    'blocGroups'=>$blocGroups,
                ...
                $addList = Bloc::find()->where(['bloc_id' => $authItems])->asArray()->all();
                foreach ($addList as $value) {
