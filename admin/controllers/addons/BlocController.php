...
class BlocController extends AController
{
    ...
    public function actionStoreList(): array
    {
        ...
        $query = BlocStore::find()->where($where)->with(['bloc', 'addons']);
        if (!empty($keywords)){
            $query->andWhere(['like', 'name', $keywords]);
        }
        $total = $query->count();
        $stores = $query->offset(($page - 1) * $pageSize)->limit($pageSize)->asArray()->all();
        $identifies = DiandiAuthAddonsCate::find()->select(['id','name'])->indexBy('id')->asArray()->all();
        $category_id_one = 0;
        if ($identifies){
            $category_id_one = array_keys($identifies)[0];
        }
        foreach ($stores as &$value) {
//            $value['create_time'] = date('Y-m-d', $value['create_time'] ?? time());
            $value['identifie'] = $value['addons'] ? $value['addons']['module_name'] : '';
            $value['logo'] = ImageHelper::tomedia($value['logo']);
            $category_id = $value['category_id'] ?: $category_id_one;
            if ($category_id && $identifies[$category_id]){
                $value['category_label'] = $identifies[$category_id]['name'];
            }
        }
    ...
    public function actionCreate(): array
    {
        ...
        if(isset($data['provinceCityDistrict'])){
