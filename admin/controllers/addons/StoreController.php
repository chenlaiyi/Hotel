...
{
    public string $modelSearchName = 'BlocStoreSearch';
    ...
    public function actionCreate(): array
   {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($model->load($data, '') && $model->save()) {
                ...
                AuthService::authStore($user_id,$model->store_id,true);
    ...
    public function actionUpdate($id): array
   {
    ...
    public function actionBlocs(): array
    {
        ...
        $list = Yii::$app->cache->get('blocs_'.$user_id);
        if ($list){
            return ResultHelper::json(200, '获取成功', (array)$list);
        }
        $list = StoreService::getAuthBlos();
        Yii::$app->cache->set('blocs_'.$user_id, $list, 3600);
        return ResultHelper::json(200, '获取成功', (array)$list);
    }
     ...
     * @param int $id
     *
     * @return array|ActiveRecord|\yii\db\ActiveRecord the loaded model
     */
    ...
    public function actionStoreCreate(): array
   {
       $data = Yii::$app->request->input();
        ...
        $extras = Yii::$app->request->input('extras',[]);
        $category_id = Yii::$app->request->input('category_id',0);
        $store = StoreService::createStore($data, $category_id, $extras);
        // 创建成功，重新返回用户权限数据
        $list = UserService::getUserMenus(0);
        $list['store'] = $store;
