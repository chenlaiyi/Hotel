...
use diandi\addons\models\form\SiteColor;
...
class ConfigController extends AController
{
    ...
    protected array $authOptional = ['site-color'];
    ...
    public function actionWechatpay(): array
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            $model->getConf($bloc_id);
            if ($model->apiclient_cert && is_file($model->apiclient_cert)) {
                $model->apiclient_cert = file_get_contents($model->apiclient_cert);
            }
            if ($model->apiclient_key && is_file($model->apiclient_key)) {
                $model->apiclient_key = file_get_contents($model->apiclient_key);
            }
    ...
    public function actionSiteColor(): array
    {
        $model = new SiteColor();
        $isGuest = Yii::$app->user->isGuest;
        if($isGuest){
            $bloc_id = WebsiteGroup::getCompanyId();
        }
        if (Yii::$app->request->isPost) {
            $bloc_id = Yii::$app->request->input('SiteColor')['bloc_id'] ?? 0;
        ...
        }else if($isGuest && !empty($bloc_id)){

            $model->getConf($bloc_id);
            $siteColor =  $model->toArray();
            if ($siteColor['id']){
                return ResultHelper::json(200, '获取成功', $siteColor);
            }
            $siteColor = require_once(Yii::getAlias('@common/config/siteColor.php'));
            return ResultHelper::json(200, '获取成功', $siteColor);
        } else {
            $bloc_id = Yii::$app->request->input('bloc_id', 0);
