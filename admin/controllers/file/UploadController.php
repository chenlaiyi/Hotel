...
class UploadController extends AController
{
    ...
    public function actionFile(): array
    {
        ...
        try {
            ...
            $is_chunk        = Yii::$app->request->input('is_chunk', 0);
            $chunk_partSize  = Yii::$app->request->input('chunk_partSize', 0);
            $chunk_partCount = Yii::$app->request->input('chunk_partCount', 0);
            $chunk_partIndex = Yii::$app->request->input('chunk_partIndex', 1);
            $md5             = Yii::$app->request->input('md5', '');
            $chunk_md5       = Yii::$app->request->input('chunk_md5', '');

            $assign_path = Yii::$app->request->input('path', '');
            ...
            //上传
            $info = $Upload::upFile($model, $is_chunk, $chunk_partSize, $chunk_partCount, $chunk_partIndex, $md5, $chunk_md5, $assign_path);
