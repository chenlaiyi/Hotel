 ...
 * @Last Modified time: 2025-06-19 14:41:57
 */

namespace admin\controllers;

use common\behaviors\HttpRequstMethod;
use common\components\actions\EditableAction;
...
class AController extends ActiveController
{
    ...
    public string $modelSearchName = 'ddUser';

    public string $ddiotUrl = '';
    ...
    public function behaviors(): array
    {
        ...
        $behaviors['corsFilter'] = [
            ...
            'cors' => [
                // restrict access to
                'Origin' => WebsiteGroup::getAllowedDomain(),
        ...
        $behaviors['access'] = [
            ...
            'allowActions' => [
                ...
                'system/index/storemenus',
                'openWeixin/basics/auth-url' //开放平台授权连接
            ],
        ];
    ...
    public function beforeAction($action): bool
    {
        ...
        loggingHelper::writeLog('adminApi', 'beforeAction', '数据操作', [
            'request' => Yii::$app->request->input(),
            'request_header' => Yii::$app->request->headers,
            'request_method' => Yii::$app->request->method,
            'request_ip' => Yii::$app->request->userIP,
            'request_userAgent' => Yii::$app->request->userAgent,
            'request_referrer' => Yii::$app->request->referrer,
            'request_isAjax' => Yii::$app->request->isAjax,
            'uid' => Yii::$app->user->identity->id ?? 0,
            'request_url' => Yii::$app->request->url,
            'data' => Yii::$app->request->input(),
        ]);

        $access_token = Yii::$app->request->headers->get('access-token', '');
        [$type,] = explode('_', $access_token);
        ...
        if ($type === 'customer') {
            Yii::$app->setComponents([
                'user' => [
                    ...
                    'enableSession' => true,
                    'loginUrl' => null,
                    'identityCookie' => ['name' => '_identity-customer-admin', 'httpOnly' => true]
                ]
            ]);
        }
     ...
     * @param $id
     *
     * @return array|ActiveRecord|\yii\db\ActiveRecord
     */
    protected function findModel($id): array|ActiveRecord|\yii\db\ActiveRecord
    {
        if (empty($id)) {
     ...
     * @return string
     */
    public function getDdiotApiUrl()
    {
        return $this->ddiotUrl;
    }
}
