 ...
 * @Last Modified time: 2025-06-20 20:01:18
 */
...
class DdApiAccessToken extends ActiveRecord implements IdentityInterface, RateLimitInterface
{
    ...
    {
        return '{{%customer_user_access_token}}';
    }
        ...
        if ($Authorization) { //用户中心授权处理
            ...
            switch ($status) {
                case 1:
                    ...
                    return $AccessTokenService->getTokenToCache($userToken['access_token'], $type);
                case -1:
            ...
            $AccessTokenService = new AccessTokenService();
            // 优化版本到缓存读取用户信息 注意需要开启服务层的cache
            return $AccessTokenService->getTokenToCache($token, $type);
        }
    }
